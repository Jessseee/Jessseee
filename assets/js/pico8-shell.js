...
let p8_allow_mobile_menu = false;
...
let pico8_buttons = [0, 0, 0, 0, 0, 0, 0, 0]; // max 8 players
...
function p8_update_button_icons() {
    ...
    let is_fullscreen=(document.fullscreenElement || document.mozFullScreenElement || document.webkitIsFullScreen || document.msFullscreenElement);

    // hash based on: pico8_state.sound_volume  pico8_state.is_paused bottom_margin left is_fullscreen p8_touch_detected
    let hash = pico8_state.sound_volume;
    ...
    let els = document.getElementsByClassName('p8_menu_button');
    for (let i = 0; i < els.length; i++) {
        ...
        let new_str = '<img width=24 height=24 style="pointer-events:none" alt="'+index.split('_')[1]+' button" src="'+p8_gfx_dat[index]+'">';
        if (el.innerHTML !== new_str)
            el.innerHTML = new_str;

        // hide all buttons for touch mode (can pause with menu buttons)
        let is_visible = p8_is_running;
...
let p8_update_layout_hash = -1;
let last_windowed_container_height = 512;
let p8_layout_frames = 0;

function p8_update_layout() {
    let canvas = document.getElementById("canvas");
    let p8_playarea = document.getElementById("p8_playarea");
    let p8_container = document.getElementById("p8_container");
    let p8_frame = document.getElementById("p8_frame");
    let csize = 512;
    let margin_top = 0;
    let margin_left = 0;
    ...
    if (is_fullscreen) {
        ...
        // never larger than window  // (happens when address bar is down in portrait mode on phone)
    ...
    if (p8_touch_detected && p8_is_running) {
        let longest_side = Math.max(window.innerWidth,window.innerHeight);
    ...
    let update_hash = csize + margin_top * 1000.3 + margin_left * 0.001 + frame_width * 333.33 + frame_height * 772.15134;
    ...
    if (!p8_is_running) {
        ...
        let el = document.getElementById("p8_start_button");
...
function p8_create_audio_context() {
    if (pico8_audio_context) {
        try { pico8_audio_context.resume(); }
        catch(err) { console.log("** pico8_audio_context.resume() failed"); }
        return;
    }

    let webAudioAPI = window.AudioContext || window.webkitAudioContext || window.mozAudioContext || window.oAudioContext || window.msAudioContext;
    if (webAudioAPI) {
        ...
        if (pico8_audio_context) {
            try {
                let dummy_source_sfx = pico8_audio_context.createBufferSource();
            ...
            catch(err) { console.log("** dummy_source_sfx.start(1, 0.25) failed"); }
        }
    }
}
...
let p8_is_running = false;
let p8_script = null;
var Module = null
function p8_run_cart(cart_path) {
    if (p8_is_running) return;
    ...
    if (p8_touch_detected) {
        let el = document.getElementById("body_0");
        let el2 = document.getElementById("p8_frame_0");
        if (el && el2) {
            el.style.display="none";
    ...
    // show touch elements
    let els = document.getElementsByClassName('p8_controller_area');
...
function p8_update_gamepads() {
    ...
    for (let i = 0; i < gps.length && i < max_players; i++) {
        let gp = gps[i];
            ...
            for (j = 0; j < gp.buttons.length; j++) {
                if (gp.buttons[j].value > 0 || gp.buttons[j].pressed) {
                    if (j < 4) {
                        ...
                        pico8_buttons[i] |= (0x10 << (((j + 1) / 2) & 1));
                    } else {
                        // PICO-8 0.2.0g: seems usually 6,7,8 are menu buttons (?) // others might be easy to accidentally bump
                        if (j >= 6 && j <= 8) pico8_buttons[0] |= 0x40; // menu button
                    }
                }
            }
        }
...
// also don't act on M / R so that you can mute / reset cart
document.addEventListener('keydown', function (event) {
    event = event || window.event;
    if (!p8_is_running) return;
    if (pico8_state.has_focus === 1)
        if ([32, 37, 38, 39, 40, 77, 82, 80, 9, 90, 88].indexOf(event.keyCode) >= 0)       // block cursors, M R P, tab, z, x
...
function p8_give_focus() {
    let el = (typeof codo_textarea === 'undefined') ? document.getElementById("codo_textarea") : codo_textarea;
    if (el) {
        el.focus();
        el.select();
    }
...
function p8_request_fullscreen() {
    ...
    let el = document.getElementById("p8_playarea");
